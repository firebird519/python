...
def filterLinks(dictList):
    # load name or mail list
    filterList = loadNameList()


    wikiHelper = WikiHelper()
    retFileHandler = open('ret.txt', 'a')

    updatedGerritCount = 0;
    ...
    for item in dictList:
        ...
        # start to filter links which not updated to wiki
        if (0 == wikiHelper.isGerritInWiki(link) and 0 == wikiHelper.isInExcludeList(link)):
            updatedGerritCount = 0
            retFileHandler.write(link + '\n')
            retList.append(link)
            print link + '\n'
        else:
            updatedGerritCount += 1
            log('gerrit updated to wiki:{0}'.format(updatedGerritCount))

        # 10 links which already updated to wiki. Not continue...
        if (updatedGerritCount > 10):
            break

    print ' '
    retFileHandler.close()

    print 'Need update link: \n'
    for link in retList:
        print link + '\n'


def parserFormattedJsonFile(fileName):
...
if len(jsonFileName) <= 0:
    ...
    if len(owner) > 0:
        jsonFileName = projectCond + '_' + branch + '_' + status + '_' + owner.replace('+',
                                                                                       '-') + '.json'
    else:
...
parserFormattedJsonFile(jsonFileName)
